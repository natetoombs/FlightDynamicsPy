...
class sensor_viewer:
    def __init__(self):
        ...
        self.plotter.define_input_vector('sensor_state', ['gyro_x', 'gyro_y', 'gyro_z', 'static_pressure', 'accel_x', 'accel_y', 'accel_z', 'diff_pressure', 'gps_n',
                                                        'gps_e', 'gps_h', 'gps_Vg', 'gps_course'])
        # plot timer
        self.time = 0.

    def update(self, sensor_state, ts):
        ## Add the state data in vectors
        # the order has to match the order in lines 72-76
        sensor_state_list = [sensor_state.gyro_x, sensor_state.gyro_y, sensor_state.gyro_z,
                           sensor_state.abs_pressure, sensor_state.accel_x, sensor_state.accel_y,
                           sensor_state.accel_z, sensor_state.diff_pressure, sensor_state.gps_n,
                           sensor_state.gps_e, sensor_state.gps_h, sensor_state.gps_Vg,
                           sensor_state.gps_course]
        self.plotter.add_vector_measurement('sensor_state', sensor_state_list, self.time)
