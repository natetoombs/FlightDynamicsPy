...
while sim_time < SIM.end_time:
    ...
    if sim_time < SIM.end_time/6:
        ...
        state.pe += -5*SIM.ts_simulation
    elif sim_time < 3*SIM.end_time/6:
        state.h += -5*SIM.ts_simulation
        state.pn += -5*SIM.ts_simulation
    elif sim_time < 4*SIM.end_time/6:
        state.psi += 0.15*SIM.ts_simulation
    elif sim_time < 5*SIM.end_time/6:
        state.theta += 0.15*SIM.ts_simulation
    else:
        state.phi += 0.15*SIM.ts_simulation
